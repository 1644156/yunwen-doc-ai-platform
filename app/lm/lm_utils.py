...
from app.utils.circuit_breaker import llm_circuit_breaker, CircuitOpenError
...
def invoke_llm(prompt: str, model: Optional[str] = None, json_mode: bool = False) -> str:
    """
    通过熔断器调用LLM（推荐使用此函数而非直接调用client.invoke）
...
def stream_llm(prompt: str, model: Optional[str] = None):
    ...
    # 熔断器只保护获取stream对象的过程
    stream = llm_circuit_breaker.call(_stream)
    for chunk in stream:
        yield chunk
