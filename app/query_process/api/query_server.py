...
class QueryRequest(BaseModel):
    ...
    enable_routing:bool = Field(True, title="是否启用自适应查询路由")
    enable_crag:bool = Field(True, title="是否启用CRAG纠错检索")


def run_query_graph(query: str, session_id: str, is_stream: bool,
                    enable_routing: bool = True, enable_crag: bool = True):
    # 一会回调用 main_graph执行
...
async def query(request: QueryRequest,background_tasks: BackgroundTasks):
    ...
    logger.info(f"query收到请求: routing={enable_routing}, crag={enable_crag}")
    ...
    if is_stream:
        ...
        background_tasks.add_task(run_query_graph, query, session_id, is_stream, enable_routing, enable_crag)
        ...
        # 同步执行
        run_query_graph(query, session_id, is_stream, enable_routing, enable_crag)
...
async def history(session_id: str,limit: int = 10):
    ...
    # chat mongodb _id -> ObjectId -> 不能直接序列化 json，需要转为 str
    items = []
    for chat in chats:
