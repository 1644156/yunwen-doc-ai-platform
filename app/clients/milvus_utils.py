...
def fetch_chunks_by_chunk_ids(
    ...
    """
    通过chunk_id主键批量查询Milvus中的切片数据（带熔断保护）
    ...
    try:
        return milvus_circuit_breaker.call(_query_batch)
    except Exception as e:
        logger.error(f"Milvus批量查询chunk_id失败：{str(e)}", exc_info=True)
        return []
...
def create_hybrid_search_requests(dense_vector, sparse_vector, dense_params=None, sparse_params=None, expr=None,
    ...
    dense_req = AnnSearchRequest(
        ...
        param=dense_params,
        expr=expr,  # expr 混合查询的查询条件 item_name (字段名) in [item_name,item_name,item_name]
        limit=limit
    )
...
def hybrid_search(client, collection_name, reqs, ranker_weights=(0.5, 0.5), norm_score=False, limit=5,
    ...
    try:
        res = milvus_circuit_breaker.call(_search)
